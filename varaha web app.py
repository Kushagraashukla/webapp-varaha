...
class MultiLabelEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for c, mlb in self.encoders.items():
            ...
            X = pd.concat([X.drop(columns=[c]), pd.DataFrame(arr, columns=new_cols, index=X.index)], axis=1)
        return X

# ---------- Load model (relative path!) ----------
# Make sure trained_model.sav is in the same folder as this script
...
shower       = st.selectbox("How Often Shower", ["daily","less frequently","more frequently","twice a day"])
heating      = st.selectbox("Heating Energy Source", ["electricity","natural gas","wood","coal"])
transport    = st.selectbox("Transport", ["public","private","walk/bicycle"])
vehicle_type = st.selectbox("Vehicle Type", ["petrol","diesel","electric","hybrid","lpg","unknown"])
social       = st.selectbox("Social Activity", ["never","sometimes","often"])
grocery      = st.number_input("Monthly Grocery Bill", min_value=0)
air_freq     = st.selectbox("Frequency of Traveling by Air", ["never","rarely","frequently","very frequently"])
...
cooking      = st.multiselect("Cooking With", ["Stove","Oven","Microwave","Grill","Airfryer"])
